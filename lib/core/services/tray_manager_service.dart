...
enum TrayMenuAction { show, settings, exit }
...
class TrayManagerService with TrayListener {
  ...
  Future<void> initialize({Function(TrayMenuAction)? onMenuAction}) async {
    ...
    try {
      await trayManager.setToolTip('Magic Printer');
      ...
      try {
        ...
        try {
          ...
          try {
            ...
            _logger.w('rootBundle não disponível ainda, tentando método alternativo: $e');
  ...
  Future<void> _updateMenu() async {
    try {
      final menu = Menu(
        items: [
          MenuItem(key: 'show', label: 'Abrir Magic Printer'),
          MenuItem.separator(),
  ...
  Future<void> setStatus(String status) async {
    await trayManager.setToolTip('Magic Printer - $status');
  }
